...
const ProjectCard = ({
  ...
  const togglePublish = async (projectId: string) => {
    ...
    console.log("Toggling publish status for project with id:", projectId);
  };
    ...
    <div key={gen.id} className="mb-4 break-inside-avoid">
      ...
      >
        ...
        >
          ...
          {!forCommunity && (
            <div className="absolute right-3 top-3 sm:opacity-0 group-hover:opacity-100 transition ">
                <div className="relative">
              <button
                onClick={(e) =>{e.stopPropagation(); setMenuOpen((prev) => !prev); }}
                className="bg-black/30 rounded-full p-1"
              >
                <EllipsisIcon className="size-6 text-gray-300" />
              </button>

              {menuOpen && (
                <ul
                  className="absolute right-0 mt-2 w-40 bg-black/80 backdrop-blur
    text-white border border-gray-500/50 rounded-lg shadow-md py-1 z-50"
                >
                  {gen.generatedImage && (
                    <a
                      href={gen.generatedImage}
                      download
                      className="flex gap-2 items-center px-4 py-2 hover:bg-black/20"
                    >
                  ...
                  {(gen.generatedImage || gen.generatedVideo) && (
                    <button
                      ...
                      className="w-full flex gap-2 items-center px-4 py-2 hover:bg-black/20"
                    >
                  ...
                  <button
                    ...
                    className="w-full flex gap-2 items-center px-4 py-2 hover:bg-red-900/20 text-red-400"
                  >
        ...
        <div className="p-4">
          ...
          {gen.productDescription && (
            <div className="mt-3">
              ...
              <div
                className="text-sm text-gray-300 bg-white/3 p-2 rounded-md
                        wrap-break-word"
              >
                {gen.productDescription}
              </div>
          ...
          {gen.userPrompt && (
            <div className="mt-3">
              <div className="text-xs text-gray-300 p-2">{gen.userPrompt}</div>
            </div>
          )}
